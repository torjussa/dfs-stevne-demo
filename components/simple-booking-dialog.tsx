...
export function SimpleBookingDialog({
  open,
  onOpenChange,
  competition,
  ...
  const registeredCount = BookingService.getCompetitionBookings(
    competition.id
  ).length;
  const availableSpots = competition.maxParticipants
    ? competition.maxParticipants - registeredCount
    : competition.totalSlots - registeredCount;
  ...
  const handlePayment = async (paymentMethod: string) => {
    ...
    // Create booking
    BookingService.createSimpleBooking(
      competition.id,
      displayName,
      user?.email || "",
      selectedClass
    ...
    // After showing success, close dialog after 2 seconds
    setTimeout(() => {
      onOpenChange(false);
      // Reset states
      setIsSuccess(false);
      setIsLoading(false);
      setReserveForFriend(false);
      setSelectedFriend("");
      setFriendQuery("");
    }, 2000);
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogPopup>
        <DialogHeader>
          ...
          <DialogDescription>
            Du melder deg på dette{" "}
            {competition.eventType === "møte" ? "møtet" : "kurset"}.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="bg-muted/50 border border-muted-foreground/20 rounded-lg p-5 mb-4">
          <div className="grid grid-cols-[auto_1fr] gap-x-4 gap-y-3">
            ...
            <div className="text-sm font-semibold">{competition.location}</div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          {isSuccess ? (
            <div className="flex flex-col items-center justify-center py-8 gap-4">
              ...
              <h3 className="text-xl font-semibold">Påmelding bekreftet</h3>
